...
struct iphdr *packet_map_ip(uint8_t *buffer, int len) {
	...
	struct iphdr *ipheader = next(ethernet, sizeof(*ethernet));
...
struct udphdr *packet_map_udp(uint8_t *buffer, int len) {
	...
	struct udphdr *udpheader = next(ip, ip->ihl * 4);
...
struct tcphdr *packet_map_tcp(uint8_t *buffer, int len) {
	...
	struct tcphdr *tcpheader = next(ip, ip->ihl * 4);
...
uint8_t *packet_map_payload(uint8_t *buffer, int len) {
	...
	if (udpheader) {
		payload = next(udpheader, sizeof(*udpheader));
	}

	struct tcphdr *tcpheader = packet_map_tcp(buffer, len);
	if (tcpheader) {
		payload = next(tcpheader, tcpheader->doff * 4);
	}
